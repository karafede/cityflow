...
filename = '2021_09_01_15h29.json'


## read sample json from Altran
with open(input_dir + filename) as json_data:
...
try:
    resp = urlrq.urlopen(url_report)
    urlrq.urlretrieve(url_report, input_dir + filename.split(".json")[0] + "_report_ALTRAN.docx")
except urllib.error.HTTPError and urllib.error.URLError:
    print("report .docx not found")
...
extension = 'json'
os.chdir('D:/Federico/CityFLows/input_files/input_bulk')
...
for json_file in filenames:
    ...
    for x in people:
        ...
        ## get ID persona
        all_people_ID = [s.split('vicini') for s in detectedpeople]
        # flattening the list and storing the result
        all_people_ID = itertools.chain(*all_people_ID)
        all_people_ID = list(all_people_ID)
        ## id persona
        people_ID = [s for s in all_people_ID if  'CxT' in s]
        ## id vicini
        closed_people_ID = [s for s in all_people_ID if "P" in s]
        # if i == 13:
        #     break
        ...
        for idx, element in enumerate(people_ID):
            if idx > 0:
                ...
                central_ID_x = int((element.split(", 'CxT':")[1]).split(',')[0])
                print('central_ID_x:', central_ID_x)
                central_ID_y = int((element.split(", 'CyT':")[1]).split(',')[0])
        ...
        ###  get all close points ("vicini")
        for idx, element in enumerate(closed_people_ID):
            if idx > 0:
                print('INPUT ---> ---> ---> closed_people_ID', idx, element)
                ## list all element one by one
                split_element = element.split('P')
                single_ELEMENT = [s for s in split_element if 'D' in s]
                for idx, element in enumerate(single_ELEMENT):
                    if idx > 0:
                        ...
                        DIST_closed_ID = element.split(", 'D':")[1]
    ...
    ## merge CENTRAL_IDs with CLOSE_IDs ("vicini")
    ...
    ## save dato to .csv
    ## DF_people[['timedate', 'ID', 'X','Y', 'CLOSE_ID', 'CLOSE_X', 'CLOSE_Y', 'DIST']].to_csv(output_dir + 'DF_people_ALTRAN_' + max(DF_people['date']) +'.txt')
    ...
    ## save dato to .csv
    all_PEOPLE[['timedate', 'ID', 'X','Y', 'CLOSE_ID', 'CLOSE_X', 'CLOSE_Y', 'DIST']].to_csv(output_dir + 'DF_people_ALTRAN_' + max(DF_people['date']) +'.txt')
...
start_date, end_date = max(DF_people['date']), min(DF_people['date'] )
# Plot daily and weekly resampled time series together
fig = plt.figure(figsize=(10,8))
ax = fig.add_subplot(111)
ax.plot(timestamps_counts.loc[start_date:end_date, 'counts'], marker='', linestyle='dotted', linewidth=0.5, label='by timestamp',  color='gray')
# ax.plot(timestamps_counts_SECs_mean.loc[start:end, 'counts'], marker='', linestyle='-', linewidth=0.5, label='by timestamp',  color='gray')
ax.plot(timestamps_counts_MIN_mean.loc[start_date:end_date, 'counts'], marker='o', markersize=6, linestyle='solid', label='minute mean resample', color='red')
ax.set_ylabel('number of people')
ax.legend()
ax.title.set_text('timeseries IDs and CLOSE IDs' + start_date)
plt.savefig(output_dir + "timeseries_people_ID_ALTRAN.png")
plt.close()
...
# plt.show()
plt.savefig(output_dir + "map_people_ID_ALTRAN.png")
plt.close()
...
plt.gca().invert_yaxis()
# plt.show()
plt.savefig(output_dir + "map_CENTRAL_ID_ALTRAN.png")
plt.close()
...
plt.savefig(output_dir + "trajectories_OD_ID_ALTRAN.png")
plt.close()
...
# plt.show()
plt.savefig(output_dir + "cluster_optimization_ALTRAN.png")
plt.close()
...
from sklearn.cluster import KMeans
kmeans = KMeans(n_clusters=3)  ### use the number of clusters found above
kmeans.fit(points)
...
plt.gca().invert_yaxis()
plt.grid(color='gray', linestyle='-', linewidth=1)
plt.savefig(output_dir + "map_CLUSTERING_CENTRAL_ID_ALTRAN.png")
plt.close()
