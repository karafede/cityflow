...
folder_trajectories_duca_aosta <- "D:/Federico/CityFLows/output_files/trajectories_duca_aosta/"
...
# DF <- read.csv('D:/Federico/CityFLows/output_files/MILANO_3_from_2021-11-04__16h6min_to_2021-11-04__16h8min.txt')[-1]
...
# DF <- read.csv('D:/Federico/CityFLows/output_files/MILANO_2_from_2021-11-04__19h0min_to_2021-11-04__19h2min.txt')[-1]
...
for (d in 1:length(day_list)) {
  ...
  # }
          ...
          for (i in list_IDs) {
            ...
            # DF_a <- DF_a %>%
            #   dplyr::mutate(time = format(ymd_hms(as.character(DF_a$timedate, tz = "CET")),'%Y-%m-%d %H:%M:%OS'))
            # DF_a <- as.data.frame(DF_a)
            ...
            if (nrow(DF_a) > 1) {
              ...
              for (j in 1:nrow(DF_a))  {
                if (j != nrow(DF_a)) {
                  ...
                  # time1a <- strptime(DF_a[j+1, "time"],format = "%Y-%m-%d %H:%M:%OS")
                  # time2a <- strptime(DF_a[j, "time"],format = "%Y-%m-%d %H:%M:%OS")

                  if(time1a != time2a) {
                    duration <- difftime(time1a,time2a)
                    ## set units
                    units(duration) <- "secs"
                    speed_j <- distance_j / as.numeric(duration) # time in seconds
                    speed_j <- abs(speed_j)
                    # print(paste0("speed: ", speed_j))

                    # DF_speed_ij <- data.frame(timedate = DF_a[j, "timedate"],
                    #                        ID = i,
                    #                        X = DF_a[j, "X"],
                    #                        Y = DF_a[j, "Y"],
                    #                        speed = speed_j)
                    # DF_people_speed_ij = rbind(DF_people_speed_ij, DF_speed_ij)
              ...
              # tryCatch({
              #   
              #   if (speed_trajectory_quarter >= speed_FWD_motion) {
              #     ##---> quick plot and trajectory
              #     p_t <- ggplot(DF_people_speed_ID, aes(x = X, y = Y, colour = ID, label=ID, group = ID)) +
              #       theme_bw() +
              ...
              #     ## save plot
              #     dpi = 96
              #     TAG <- paste0("ID_", i, "_mean_speed_",round(speed_trajectory_quarter, digits = 2),"__",
              #                   year_list_quarter_a[k], "_", month_list_quarter_a[k], "_", day_list_quarter_a[k],"__",
              #                   list_hours[h],
              #                   "h", quarters[q], "min")
              #     # ggsave(filename=paste0(folder_trajectories_duca_aosta,"Trajectory_",TAG, ".png"),
              #     #        width = 700 / dpi, height = 650 / dpi,
              #     #        dpi = dpi)
              #     
              #   }
              # }, error= function(err) {print("no forward motion")
              #   
              # }, finally = {
              #   
              # })
...
# write.csv(all_ID_speed, file = paste0("D:/Federico/CityFLows/output_files/all_ID_speeds_duca_aosta_angles_4Nov_50ms.csv"), row.names=FALSE)
write.csv(all_ID_speed, file = paste0("D:/Federico/CityFLows/output_files/all_ID_speeds_lato_taxi_angles__17h20_to_18h20_10Oct2021.csv"), row.names=FALSE)



# write.csv(all_ID_speed, file = paste0("D:/Federico/CityFLows/output_files/all_ID_speeds_duca_aosta_angles_c.csv"), row.names=FALSE)
# write.csv(all_ID_speed, file = paste0("D:/Federico/CityFLows/output_files/all_ID_speeds_lato_taxi_angles_c.csv"), row.names=FALSE)
# write.csv(all_ID_speed, file = paste0("D:/Federico/CityFLows/output_files/all_ID_speeds_lato_biciclette_angles_c.csv"), row.names=FALSE)
